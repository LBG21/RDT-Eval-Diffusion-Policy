...
from diffusion_policy.dataset.hdf5_vla_dataset import HDF5VLADataset
...
def visual_actions(action, name):
    ...
    for i in range(14):
        row = i % 7
        col = i // 7
        axs[row, col].plot(time_steps, action[:, i])
        axs[row, col].set_title(f'dim {i+1}')
...
class RobotWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        def custom_collate_fn(batch):
            batch_obs_agent_pos = torch.stack([item['obs']['agent_pos'] for item in batch])
            batch_obs_head_cam = torch.stack([item['obs']['head_cam'] for item in batch])
            batch_actions = torch.stack([item['action'] for item in batch])
            return {
                'obs': {
                    'agent_pos': batch_obs_agent_pos,
                    'head_cam': batch_obs_head_cam,
                },
                'action': batch_actions,
            }
        
        config_path ='/data/lingxuan/RoboTwin/policy/Diffusion-Policy/diffusion_policy/configs/base.yaml'
        dataset = HDF5VLADataset()
        # train_dataloader = create_dataloader(dataset, **cfg.dataloader)
        train_dataloader = DataLoader(dataset, collate_fn=custom_collate_fn, **cfg.dataloader)
        # normalizer = dataset.get_normalizer()

        # configure validation dataset
        # val_dataset = dataset.get_validation_dataset()
        val_dataset = HDF5VLADataset()
        # val_dataloader = create_dataloader(val_dataset, **cfg.val_dataloader)
        val_dataloader = DataLoader(val_dataset,  collate_fn=custom_collate_fn, **cfg.val_dataloader)
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                #         # visual_actions(pred_action[0].cpu().numpy(), f'pred_action_{self.epoch}')
                #         # visual_actions(gt_action[0].cpu().numpy(), f'gt_action_{self.epoch}')
                #         mse = torch.nn.functional.mse_loss(pred_action, gt_action)
                #         step_log['train_action_mse_error'] = mse.item()
                #         print(f"Sampling loss: {mse.item()}")
                #         del batch
                #         del obs_dict
                #         del gt_action
                #         del result
                ...
                if ((self.epoch + 1) % cfg.training.checkpoint_every) == 0:
                    ...
                    self.save_checkpoint(f'checkpoints/{save_name}_predict6/{self.epoch + 1}.ckpt') # TODO
